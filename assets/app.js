...
// Updated for LLM result
function updateFeedback(detection) {
    const llmPlaceholder = document.getElementById('llm-placeholder');
    const llmDisplay = document.getElementById('llm-display');

    // do not let the feedback overwrite LLM feedback
    if (llmDisplay && llmDisplay.style.display === 'flex') {
        return; 
    }
    ...
    if (detection && objectInfo[detection.content]) {
        ...
        // update the placeholder area instead of overwriting the whole container
        if (llmPlaceholder) {
            llmPlaceholder.innerHTML = `
